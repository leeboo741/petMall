  ...
  methods: {
      ...
      for (let i = 0; i < this.data.imagePathList.length; i++) {
        ...
        if (!Utils.checkIsVideo(tempImagePath)) {
          if (Utils.checkIsObject(tempImagePath)) {
            tempPreviewImageList.push(tempImagePath.fileAddress ? tempImagePath.fileAddress : tempImagePath.coverAddress);
          } else {
    ...
    deleteImageAction: function (e) {
      ...
      let myEventDetail = {
        ...
        uploadImageId: this.data.uploadImageId,
      }; // detail对象，提供给事件监听函数
      let myEventOption = {
          ...
          if (res.tapIndex == 0) {
            ...
            tempImageParam.success = function success(res) {
              ...
              let myEventDetail = {
                ...
                uploadImageId: that.data.uploadImageId
              }; // detail对象，提供给事件监听函数
              let myEventOption = {
                'bubbles': false, // 事件是否冒泡
                'composed': false, // 事件是否可以穿越组件边界，为false时，事件将只能在引用组件的节点树上触发，不进入其他任何组件内部
    ...
    */
    uploadListImage: function (uploadCompleteCallback) {
      ...
      tempParam.successCallback = function successCallback(res) {
         ...
         */
        that.uploadSuccess(res.root[0].fileAddress ? res.root[0].fileAddress : res.root[0].coverAddress, res.root[0]);
      };
      // 上传失败回调
