// pages/order/unsendOrderList/index.js
...
Page({
  ...
  tapToOrderDetail: function (e) {
    ...
    let type = this.data.currentTabIndex==0?'pet':'item';
    let str = PagePath.Page_Order_Detail + "?orderno=" + tempOrderNo + "&type=" + type;
    Utils.logInfo(str)
    wx.navigateTo({
      url: PagePath.Page_Order_Detail + "?orderno=" + tempOrderNo + "&type=" + type,
    })
  },
   ...
   * 点击确认发货
   */
  tapConfirmSend: function (e) {
    let index = e.currentTarget.dataset.index;
    let tempOrder = this.data.dataSource[index];
    app.globalData.confirmSendOrder = tempOrder;
    wx.navigateTo({
      url: PagePath.Page_Order_UnsendList_ConfirmSend,
    })
  },
    ...
    UserService.isLogin(function isLoginCallback(){
      ...
      if (that.data.currentRole == 0) { //买家
        if (that.data.currentTabIndex == 0) { //宠物
          OrderService.customerQueryBeShipped(param,
        ...
        } else {    //商品userGoodOrderToBeShipped
          OrderService.userGoodOrderToBeShipped(param,
            function queryResultCallback(result) {
              if (Util.checkIsFunction(getDataCallback)) {
                getDataCallback(result.root)
              }
        ...
        if (that.data.currentTabIndex == 0) { //宠物 
          OrderService.businessQueryBeShipped(param,
            function queryResultCallback(result) {
              if (Util.checkIsFunction(getDataCallback)) {
                getDataCallback(result.root)
              }
          ...
          OrderService.businessGoodOrderToBeShipped(param,
            function queryResultCallback(result) {
              if (Util.checkIsFunction(getDataCallback)) {
                getDataCallback(result.root)
              }
